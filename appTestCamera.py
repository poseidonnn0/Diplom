...
# Конфигурация камеры
CAMERA_INDEX = 0  # Индекс USB-камеры (обычно 0 для первой подключенной камеры)
DEFAULT_RESOLUTION = (1920, 1080)  # Резервные значения разрешения

# Инициализация параметров видео
cap = cv2.VideoCapture(CAMERA_INDEX)
if not cap.isOpened():
    logging.error("Ошибка инициализации камеры. Используются резервные значения разрешения.")
...
def video_processing():
    ...
    try:
        ...
        if not cap.isOpened():
            logging.error("Error opening video device")
            return
        ...
        while True:
            ...
            for zone in zones_config:
                ...
                cv2.putText(display_frame,
                            f"{zone['name']}: {last_counts[zone['name']]}",
                            (x1 + 10, y1 + 30),
